...
export const minimal: ThemeDefinition = {
  name: 'minimal',
  label: 'Minimal',
  cssClass: 'ef-t-minimal',
  blocks: {
    hero: (b) => `
      <h1 class="m-hero-title">${b.title}</h1>
      <p class="m-hero-sub">${b.subtitle || ''}</p>
      <p class="m-hero-desc">${b.description || ''}</p>
      ${cta(b)}`,

    features: (b) => {
      const heading = b.heading ? `<h2 class="m-heading">${b.heading}</h2>` : '';
      const items = (b.items || []).map((item: any) =>
        `<div class="m-feat"><strong>${item.title}</strong><span class="m-dim">${item.description || ''}</span></div>`
      ).join('');
      return `${heading}<div class="ef-features">${items}</div>`;
    },

    form: (b) => {
      const heading = b.heading ? `<h2 class="m-heading">${b.heading}</h2><p class="m-dim">${b.description || ''}</p>` : '';
