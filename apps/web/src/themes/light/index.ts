...
export const light: ThemeDefinition = {
  ...
  blocks: {
    hero: (b) => `
      <h1 class="l-hero-title">${b.title}</h1>
      <p class="l-hero-sub">${b.subtitle || ''}</p>
      <p class="l-hero-desc">${b.description || ''}</p>
      ${cta(b)}`,

    features: (b) => {
      const heading = b.heading ? `<h2 class="l-heading">${b.heading}</h2>` : '';
      const cols = b.columns || 0;
      const colStyle = cols ? ` style="grid-template-columns:repeat(${cols},1fr)"` : '';
      const items = (b.items || []).map((item: any) =>
        `<div class="l-feat-card">${tag(item)}<h3>${item.title}</h3><p>${item.description || ''}</p></div>`
      ).join('');
      return `${heading}<div class="ef-features l-feat-grid"${colStyle}>${items}</div>`;
    },

    form: (b) => {
      const heading = b.heading ? `<h2 class="l-heading">${b.heading}</h2><p class="l-dim">${b.description || ''}</p>` : '';
